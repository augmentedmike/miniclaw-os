...
| **openclaw.json** | Config file present at `$OPENCLAW_DIR/openclaw.json` | Creates minimal config with Python |
...
The tool resolves `jobs.json` from `$OPENCLAW_STATE_DIR/cron/jobs.json`, falling back to `~/am/cron/jobs.json`.
