...
function sendMessage(text, imagePaths) {
  if (!proc || !ready) {
  ...
  // When images are attached, prepend instructions for Claude to read the image files
  if (imagePaths && imagePaths.length > 0) {
    const imageRefs = imagePaths.map(p => `  - ${p}`).join("\n");
    const imagePrompt = `[The user has attached ${imagePaths.length} image${imagePaths.length > 1 ? "s" : ""}. Please read ${imagePaths.length > 1 ? "each of these" : "this"} image file${imagePaths.length > 1 ? "s" : ""} to see what they shared:\n${imageRefs}\n]\n\n`;
    proc.stdin.write(imagePrompt + text + "\n");
  } else {
  ...
  conn.on("data", (chunk) => {
    ...
    for (const line of lines) {
      ...
      try {
        ...
        if (msg.type === "message" && msg.text) sendMessage(msg.text, msg.imagePaths);
