...
export interface Attachment {
  ...
  label?: string;
  mime?: string;
  type?: string;       // e.g. 'research_report', 'web_search', 'file'
  ref_id?: number;     // optional reference to source DB record (e.g. research.db id)
  created_at: string;
}
...
export interface Card {
  ...
  depends_on?: string[];          // optional — list of card IDs this card depends on (blocked by)
  created_at: string;
  updated_at: string;
...
export interface WorkLogEntry {
  ...
  worker: string;   // agent/worker ID
  note: string;     // what was done
  links?: string[]; // PR/commit/branch URLs
}

const PRIORITY_RANK: Record<Priority, number> = { critical: 0, high: 1, medium: 2, low: 3 };

/**
 * Canonical card sort order — used by both CLI and web.
 * 1. focused (has "focus" tag) — always at top
 * 2. active (currently being worked on, caller supplies activeIds)
 * 3. priority (critical → high → medium → low)
 * 4. age (oldest first)
 */
export function sortCards(cards: Card[], activeIds?: Set<string>): Card[] {
...
export function parseCard(content: string): Card {
  ...
  const depends_on = Array.isArray(fm.depends_on)
    ? (fm.depends_on as string[])
    : (fm.depends_on ? String(fm.depends_on).split(",").map(s => s.trim()).filter(Boolean) : undefined);
  ...
  return {
    ...
    history,
    ...body,
    verify_url: String(fm.verify_url ?? ""),
    work_log: [],
    attachments: [],
  };
}
...
export function serializeCard(card: Card): string {
  ...
  const dependsOnLine = card.depends_on && card.depends_on.length > 0
    ? `depends_on: [${card.depends_on.join(", ")}]\n`
    : "";
