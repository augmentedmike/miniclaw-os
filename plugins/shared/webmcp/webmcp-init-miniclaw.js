...
document.addEventListener('DOMContentLoaded', function () {
  ...
  (function () {
    ...
    var chatWs = null;
    var pendingRequests = new Map(); // requestId → { resolve, timer }
    ...
    function handleChatMessage(event) {
      try {
        ...
        if (data.type === 'result') {
          // Match by requestId if provided, otherwise resolve the oldest pending
          ...
          if (reqId && pendingRequests.has(reqId)) {
            ...
            // Fallback: resolve the earliest pending request (FIFO)
    ...
    // Observe chat widget — connect via data attribute or MutationObserver
    function observeChat() {
      // Method 1: Check for existing chat widget with WS URL
      ...
      // Method 2: Watch for chat widget appearing in the DOM
      ...
      // Stop observing after 30s if no chat widget found
