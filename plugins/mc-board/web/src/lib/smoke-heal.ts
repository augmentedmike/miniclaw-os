 ...
 * automatically. Duplicate cards are prevented by checking for existing
 * smoke-fix cards with the same component/label before creating new ones.
 */
...
const REMEDIATIONS: Remediation[] = [
  ...
  { match: "google chrome not found", component: "browser", plan: "Install Chrome: brew install --cask google-chrome" },
...
/**
 * Build a canonical key for a smoke failure to detect duplicates.
 * Uses component + normalized label so re-runs don't create redundant cards.
 */
function failureKey(failure: SmokeFailure): string {
  return `${failure.component}::${failure.label}`.toLowerCase().replace(/\s+/g, " ").trim();
}

/**
 * Query existing smoke-fix cards and return a set of failure keys that
 * already have an open card (not in 'done' or 'archived' columns).
 */
function getExistingFixCardKeys(): Set<string> {
  const ocBin = findBin("openclaw");
  if (!ocBin) return new Set();

  const result = spawnSync(ocBin, ["mc-board", "board", "--json"], {
    encoding: "utf-8",
    timeout: 15_000,
  });
  ...
  try {
    ...
    for (const card of list) {
      ...
      if (!tags.includes("smoke-fix")) continue;
      if (card.col === "done" || card.col === "archived") continue;

      // Extract component and label from the title pattern "Fix: <component> — <label>"
      const m = (card.title || "").match(/^Fix:\s*(.+?)\s*—\s*(.+)$/);
      if (m) {
        const key = `${m[1]}::${m[2]}`.toLowerCase().replace(/\s+/g, " ").trim();
        keys.add(key);
      }
    }
  } catch { /* parse error — assume no existing cards */ }
...
/**
 * Process mc-smoke output: parse failures, create fix cards (skipping duplicates),
...
export async function healSmokeFailures(
  ...
  if (failures.length === 0) {
    return { failures: [], cards: [], skippedDuplicates: 0, notified: false };
  }

  // Check for existing smoke-fix cards to prevent duplicates
  const existingKeys = getExistingFixCardKeys();
  ...
  for (const failure of failures) {
    ...
    if (card) {
      cards.push(card);
      // Add to existing set so we don't create duplicates within this batch
  ...
  return { failures, cards, skippedDuplicates, notified };
}
