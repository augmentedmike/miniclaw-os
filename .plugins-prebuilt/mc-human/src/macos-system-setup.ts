 ...
 * - Activate via kickstart with observe-only access
 *
 * Security: uses -privs -ControlObserve (minimum needed for VNC observation)
 * instead of -privs -all which would grant full remote control, clipboard,
 * and file transfer privileges.
 *
 * Security: uses -allowAccessFor -specifiedUsers instead of -allUsers.
 * For single-user Mac mini deploys where -allUsers would be acceptable,
 * change to -allowAccessFor -allUsers and document the justification.
 */
async function setupScreenSharing(logger: Logger): Promise<StepResult> {
  ...
  // Step 1b: Activate via kickstart with tightened permissions
...
export async function setupMacOsSystem(logger: Logger): Promise<SetupResult> {
  ...
  let summary =
    ...
    steps.map((s) => `  ${s.success ? "OK" : "FAIL"} ${s.name}\n      ${s.success ? s.output : s.error}`).join("\n");
